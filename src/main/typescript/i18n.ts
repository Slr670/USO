 ...
 */
const I18N_RESOURCES: I18nResources = {
    en: {
        translation: {
            hero: {
 ...
 * Module Keys Mapping
 */
const MODULE_KEYS: readonly string[] = ['m1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8'];
 ...
 */
function setStoredLanguage(lng: LanguageCode): void {
    try {
 ...
 */
function initI18nEngine(): void {
    const savedLang: LanguageCode = getStoredLanguage();
    ...
    if (i18n) {
        if (reactI18next && reactI18next.initReactI18next) {
            (i18n as unknown as { use: (plugin: unknown) => void }).use(reactI18next.initReactI18next);
        }
        ...
        (i18n as unknown as { on: (evt: string, cb: (lng: string) => void) => void }).on('languageChanged', (lng: string) => {
            const validLng: LanguageCode = (lng === 'th' || lng === 'en') ? lng : 'en';
...
function t(key: string, options: Record<string, unknown> = {}): string {
    ...
    const currentLang: LanguageCode = getStoredLanguage();
    const bundle = I18N_RESOURCES[currentLang] ?? I18N_RESOURCES.en;
    const parts = key.split('.');
    let val: unknown = bundle.translation;
 ...
 */
function changeLanguage(lng: LanguageCode): void {
    const i18n = typeof window !== 'undefined' ? window.i18next : undefined;
 ...
 */
function applyTranslations(lng?: LanguageCode): void {
    const currentLng: LanguageCode = lng ?? getStoredLanguage();
    if (typeof document !== 'undefined') {
        ...
        if (Array.isArray(menuData)) {
            menuData.forEach((item, index) => {
                ...
                if (mKey) {
                    const mutableItem = item as unknown as Record<string, string>;
            ...
            const cardButtons = document.querySelectorAll<HTMLButtonElement>('.card-btn');
            cardButtons.forEach((btn, index) => {
                const labelSpan = btn.querySelector<HTMLSpanElement>('.card-label-text');
                const mKey = MODULE_KEYS[index];
                if (labelSpan && mKey) {
 ...
 */
function applyFallbackTranslations(lng: LanguageCode): void {
